...
Приложение использует простую, но безопасную схему аутентификации и регистрации на основе JWT и Redis:
...
1. **Получить CAPTCHA**  
   `GET /api/v1/captcha` → возвращает изображение и `captchaId`.

2. **Решить CAPTCHA**  
   `POST /api/v1/captcha/solve` → возвращает временный `captchaToken` (срок жизни — 2 минуты).

3. **Зарегистрироваться**  
   `POST /api/v1/auth/register` с `captchaToken` в теле → создаётся аккаунт.

4. **Войти**  
   `POST /api/v1/auth/login` → выдаются `accessToken` и `refreshToken`.

5. **Работать с API**  
   Все защищённые запросы содержат заголовок:  
   `Authorization: Bearer <accessToken>`

6. **Обновить сессию**  
   При истечении срока действия `accessToken` — отправьте `refreshToken` на `/api/v1/auth/refresh` → получите новую пару токенов.

7. **Выйти из системы**  
   `POST /api/v1/auth/logout` → `refreshToken` удаляется из Redis.  
   (Access token остаётся валидным до истечения срока — это нормально для stateless JWT.)
...
- **Lombok** и **record-классы** — минимизация шаблонного кода
...
- 🌐 **Совместимо с микросервисами**:  
  работает с **Spring Cloud Gateway**, поддерживает **Redis Cluster**.
